    ...
    <div class="col-md-12">
        <div class="card ">
            ...
            <div class="card-body ">
                ...
                @endif
                <form method="POST" action="{{ route('reports.totalSales') }}">
                    @csrf
                    <div class="row">
                    ...
                    <table id="orders-table" class="table table-striped table-hover" cellspacing="0" width="100%" style="width:100%">
                        ...
                        <tbody>
                            ...
                            @foreach ($orders as $date => $order)
                                ...
                                <tr>
                                    <td>{{ $date }}</td>
                                    <td>{{ $order->min->number }}</td>
                                    <td>{{ $order->max->number }}</td>
                                    <td>{{ $order->count() }}</td>
                                    <td>{{ number_format($order->sum->total) }}</td>
                                    <td>{{ $order->sum->discount }}</td>
                                    <td>{{ $order->sum->extra }}</td>
                                    <td>{{ $order->sum->tax }}</td>
                                    <td>{{ number_format($order->sum->total - $order->sum->discount + $order->sum->extra - $order->sum->tax) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
                <div class="lead muted text-right m-4">Total: {{ number_format($totalOfFinalTotals) }}</div>
            </div>
        @endif
    </div>
</div>
